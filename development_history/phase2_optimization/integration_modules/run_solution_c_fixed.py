...
try:
    ...
    # 动态导入
    integration_file = os.path.join(current_dir, "integration", "openspace_deep_integration.py")
    if os.path.exists(integration_file):
        print(f"✅ 找到集成模块: {integration_file}")
        
        import importlib.util
        spec = importlib.util.spec_from_file_location("openspace_deep_integration", integration_file)
        ...
        if isinstance(planning_result, dict) and 'tasks' in planning_result and planning_result['tasks']:
            ...
            if isinstance(quantitative_result, dict):
                ...
                if 'test_cases' in quantitative_result:
                    print(f"   生成测试用例: {len(quantitative_result['test_cases'])}个")
